...
public:
  ...
  float       GetNextParameterFloat(){ cout << m_bGoodFile << '\n'; if (m_bGoodFile) return (float)atof(GetNextParameter().c_str());throw std::runtime_error("bad file");}
  int         GetNextParameterInt(){ cout << m_bGoodFile << '\n'; if (m_bGoodFile) return atoi(GetNextParameter().c_str());throw std::runtime_error("bad file");}
